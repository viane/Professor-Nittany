...
// Change below Variables to change chat window popup size
var ChatWindow_Height = 600;
var ChatWindow_Width = 400;
...
function StartCheck() {
    ...
    if (Name) {
        Name.value = document.getElementById("txtFirstName").value + " " + document.getElementById("txtLastName").value;
        tName = Name.value;
    }
    ...
    strsrc = sTrackURL + "/Webtrak.aspx?Fn=Start&SetCookie=True" + QryStr + "&num=" + ran();
...
function CheckOnTime() {
    ...
    if (vIntervalIDExists == true || cHeight != 1 || cWidth != 1)
        return;
    ...
    if (cHeight != 1 || cWidth != 1) {
        var strsrc = sTrackURL + "/Webtrak.aspx?fn=CheckChat&num=" + ran();
        DefImage.src = strsrc;
        return;
    }
    else {
...
function EstimateTime() {
    var strurl = "";
    var retValue = "False";
    var querystr = sChatURL + "/ChatActions/ChatAction.ashx";
...
function StartChecking() {
    window.setTimeout("StartCheck()", 250); // Change this interval for the delay to show sitevisitor row at agent side.
    return;
}
...
function CreateChatNow() {
    //Creating Html and Adding it to the Div ChatNow and then Showing up the Div of TreatCodeSample.htm
    var chatDiv = document.getElementById('ChatNow');
    chatDiv.innerHTML = "<div style='text-align:right;padding-top:15px'><p>Our customer service representative will come online to assist you. Please wait...</p>"
	                    + "<input type='button' id='btnOk' class='cssButtton ui-button ui-widget ui-state-default ui-corner-all' name='Ok' value='Ok' onclick='javascript:return InitiateChat(true);'/>"
	                    + "</div>";
    $("#ChatNow").attr('title', TL_PROACTIVE_TITLE);
    $("#ChatNow").dialog({ resizable: false, modal: true, height: 270,
        close: function (event, ui) { DenyProactive(); }
    });
...
function DenyProactive() {
    var querystr = sChatURL + "/ChatActions/ChatAction.ashx?ProactiveDeny=REMOVEPROACTIVE";
...
function InitiateChat(proactive) {
    OpenChat(proactive);
    // To unregister close event, since its not a deny on close
    $("#ChatNow").dialog({ close: function (event, ui) { } });
    //Closing the dialogue box
    $("#ChatNow").dialog("close");
}
...
function OpenChat(proactive) {
    ...
    var theName = '';
    if (Name && Name.value != '') {
        theName = GetTrimmedString(Name.value);
        Name.value = theName;
    }
    ...
    dataLayer.push({
        'event': 'talismaChat',
        'eventCategory': 'Talisma Chat',
        'eventAction': 'Start Chatting'
    });
    var win = window.open("about:blank", "chat", "height=" + ChatWindow_Height + ", width = " + ChatWindow_Width);
    var url = '';
    //var md = new MobileDetect(window.navigator.userAgent);
    /*
     *  If the user is on a mobile device, set the URL to mobile Visitor Chat.
     *  else set the URL to desktop visitor chat.
     */
    var environment = Drupal.settings.psu_talismachat.environment;
    if(document.getElementById("boolIsMobile").value) {
      url = environment + "/Media/MVisitorChat/StartChat.aspx"; // Mobile visitor chat
    }
    else {
      url = environment + "/Media/VisitorChat/StartChat.aspx"; // Desktop visitor chat
    }

    var form = document.getElementById("admissions-chat-request-form");
    form.action = url;
    form.submit();
}
...
;
function ShowEmailUs(e) {
	var keyCode = e.keyCode || e.which;

	if (keyCode != 9) {
		document.getElementById('chat-body').style.display='none';
		document.getElementById('block-webform-client-block-1712').style.display = 'block';
	        dataLayer.push({
	            'event': 'talismaChat',
	            'eventCategory': 'Talisma Chat',
	            'eventAction': 'Email Us'
	        });
			if (document.getElementById("confirmation-email-us").hasChildNodes()) {
				document.getElementById('confirmation-email-us').style.display = 'none';
				document.getElementById('visible_float').style.bottom = 0;
				}
	    var WebformName = document.getElementById("txtFirstName").value + " " + document.getElementById("txtLastName").value;
	    document.getElementById("edit-submitted-name").value = WebformName;
	    var WebformEmail = document.getElementById("Email").value;
	    document.getElementById("edit-submitted-email").value = WebformEmail;
	}
}

function ShowChatRequest() {
	HideEmailUsAndChatRequest();
	document.getElementById('chat-body').style.display='block';
  document.getElementById('talismaheader-icon-close').style.display = 'block';
	if (!document.getElementById("confirmation-email-us").hasChildNodes() || document.getElementById('visible_float').style.bottom < 0) {
		document.getElementById('visible_float').style.bottom = 0;
	}
	document.getElementById('block-webform-client-block-1712').style.display = 'none';
        dataLayer.push({
            'event': 'talismaChat',
            'eventCategory': 'Talisma Chat',
            'eventAction': 'Expand'
...
function HideEmailUsAndChatRequest() {
	document.getElementById('chat-body').style.display='none';
	document.getElementById('block-webform-client-block-1712').style.display = 'none';
    document.getElementById('talismaheader-icon-close').style.display = 'none';
}

function TalismaChatClick(e) {
	var keyCode = e.keyCode || e.which;
...
jQuery(document).ready(function ($) {
  if ($("#confirmation-email-us").length > 0) {
    $("#block-psu-talismachat-talisma-chat").click(function(){$('#confirmation-email-us').hide();});
		setTimeout( function() {
	    $("#confirmation-email-us").addClass('element-invisible');
		}, 15000 );
  }
});
;
